...
def init_argparse()  -> argparse.ArgumentParser:
    ...
    volume_slider.add_argument(
        "--increase", "-i", type=int, dest='vol_up')
    volume_slider.add_argument(
        "--decrease", "-d", type=int, dest='vol_down')
    parser.add_argument(
        "--mute", "-m", action='store_true')
    parser.add_argument(
...
def get_volume() -> (int, str):
    volume = subprocess.run(["wpctl", "get-volume", "@DEFAULT_AUDIO_SINK@"], capture_output = True, text = True).stdout.split()
...
def notify_volume(volume: int, message: str=None):
    ...
    nf.set_hint("x-canonical-private-synchronous", GLib.Variant('s', "volume-notification"))
